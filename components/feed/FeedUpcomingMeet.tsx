 ...
 * community feed. Uses the shared `.feed-card` two-column shell (avatar left,
 * content right) so it sits in the same rhythm as post cards instead of as a
 * standalone bordered card. The left column carries the **group avatar** (the
 * meet's host community, circle per Avatar Rule B); the right column keeps the
 * meet anatomy (type pill + "Coming up" → title → date → location → group →
 * attendee stack). See `docs/features/meets.md` → Meet-card anatomy.
 */
...
export function FeedUpcomingMeet({ meet }: { meet: Meet }) {
  ...
  return (
    <Link
      href={`/meets/${meet.id}`}
      className="feed-card feed-card--meet"
      style={{ textDecoration: "none", display: "block" }}
    >
      <div className="feed-card-body">
        {/* Left column: host-group avatar (circle — Avatar Rule B), so the card
            aligns with post cards. Falls back to initials when the group has no
            cover, or to the meet-type icon when the meet has no group. */}
        <div className="feed-card-col-avatar">
          {group?.coverPhotoUrl ? (
            ...
            <DefaultAvatar name={group.name} size={40} className="feed-card-avatar" />
          ) : (
            <span className="feed-card-avatar feed-card-avatar--meet-icon">
              {MEET_ICONS[meet.type]}
            </span>
          )}
        </div>

        {/* Right column: the meet anatomy */}
        <div className="feed-card-col-content">
          {/* Type pill + "Coming up" context badge */}
