...
## Current Phase: Inbox & Notifications (opened 2026-05-08)

**Phase board:** `docs/phases/inbox-and-notifications-deep-pass.md`. Seven workstreams: A) notification delivery wiring (real `session_started` / `session_completed` fires), B) finish inquiry-driven trust transitions (first-service-message → mutual Familiar — Appointment edge case), C) collapse NotificationsPanel into `/notifications` page (one surface, mobile-first), D) inbox visual + UX hygiene (density, "You:" prefix, name + dog data normalization), E) messaging-stream design questions (timestamps, edit-after-submit on visit reports w/ lean defaults, superseded proposal cards), F) cleanup deferred from Pricing & Proposals close (delete dead ContractCard + CSS + `type: "contract"`), G) walkthrough audit findings (schedule label adapt, /bookings strip not firing for fresh seals, missing inline review CTA on Sessions tab). Notifications context grows an `addNotification` API persisted via `usePersistedState("doggo-notifications")`.
