...
| C8 | **(New, from C1 investigation.)** Sweep the broader dead old-inbox + dead page-shell CSS — ~40 zero-consumer classes (`.inbox-page`/`-header`/`-list`/`-row*`/`-msg*`/`-composer*`/`-contact*`/`-empty*`/`-detail`/`-messages` + `.bookings-page-shell`/`.notifications-page-shell` non-md-shell rules + `.discover-page-shell`). Interleaved with LIVE classes (`inbox-nav-search*`, `inbox-thread-*`, `inbox-message*`, `inbox-proposal*`, `inbox-direct-empty`) so it needs class-by-class boundary work. | [[design-system]] | todo |
