...
| A5 | **Tereza mock data — mixed-roster demo case.** Add an optional service link to one of Tereza's hosted park walks (her Sunday park walk). She offers "Group walk" Care service at 200 Kč; the walk stays free to join but paid attendees get her dog-walking attention. Demos the optional-link configuration end-to-end. | **todo** — ⚠️ board wording says "Care service" but only Meet-type services carry `linkedMeetIds` (A1/A6). Per the locked model the optional-link service must be `kind: "meet"` (a paid group walk = Meet-type per Services-as-Catalog). Treat A5's service as Meet-type when building. |
