...
| H4 | **Link-authoring UI.** A surface for a carer/host to *create* a meet↔Care-service link. | **done (single-carer)** — resolved to a **carer-side picker**: the Care service edit card (`walks_checkins` only — other Care types happen at a home, not on a meet) has an "Offered on these meets" CheckboxRow list of the carer's free walk meets. `CarerCareServiceConfig` gained an optional `linkedMeetIds` (carer-side half of the two-sided link); profile Save flushes via `syncMeetLinksForService`. Verified — toggle + Save + re-Edit round-trips. **Multi-carer** linking (a meet host advertising *other* carers' services) needs a meet-side surface — remains the follow-on. |
