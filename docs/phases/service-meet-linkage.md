...
> **⚠️ Remodel note (2026-05-17) — config #2 corrected.** Mid-walkthrough a miscategorisation surfaced: a drop-off "Group walk" is a **Care** service ("I take the dog"), not a Meet-type service. Config #2 ("Meet + optional service / mixed roster") was built on that error. **Corrected** (see Open Q §13 correction note): config #2 is "a *free* meet + linked drop-off **Care** services, where **booking ≠ attending**" — two separate paths (join free as a walker / book a carer to walk your dog), never both. The phase's `tereza-group-walk` Meet-type service, `meet-15.linkedServices`, and `LinkedServiceCallout` were removed; `meet-15` reverted to a plain free walk (config #1). Tasks **A5, G2, D2** and parts of **D1 / D3 / D4** are superseded — see their status cells. The corrected config #2 build (Care services made meet-linkable + a drop-off-from-meet booking flow + multi-service / multi-carer links) is a **scheduled follow-on phase** (ROADMAP → Upcoming). What the phase still delivers stands: the authoring UI (Workstream B), the required-link config (#3 — Klára's training), the free-unlinked config (#1), and the Meet-service booking flow (Workstream C).
