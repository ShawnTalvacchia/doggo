...
| # | Description | Status |
|---|-------------|--------|
| D1 | Meet card chrome handles three states: (a) free unlinked (today's behavior — "Going / Skip / Interested"), (b) optional service link (primary "Join free" + inline service callout "Have your dog walked: 300 Kč →"), (c) required service link ("Book session → 350 Kč" sole CTA). Implement in `CardMeet` + meet detail page. | **done (meet detail)** — the meet detail covers all three states: free unlinked (normal RSVP), optional (free RSVP + D2 callout), required (`serviceCTA` "About this service" card, no free RSVP). **`CardMeet` list-card chrome for optional-link meets deferred** — would add carer-resolution to a hot list component for one edge case; `CardMeet` already shows the `serviceCTA` price for required/legacy meets. |
| D2 | Linked-service callout inline UI — sits below the meet meta (when / where / who), above the action row. Treatment: subtle bordered box with carer avatar + service title + price + chevron. Tap routes into `BookSessionSheet` pre-seeded with this meet's occurrences. | **done** — `components/meets/LinkedServiceCallout.tsx`; rendered below the when/where card on the meet detail for optional-link meets (linked service, no legacy `serviceCTA`). Verified on `meet-15`. |
| D3 | **Roster rendering for mixed-roster meets.** People tab on the meet shows all attendees (free + paid) the same way — no visible distinction between "free RSVP" and "service booking" attendees. Behind the scenes the booking record exists for paid ones (carer sees it on their dashboard), but the social surface stays unified. Verify People tab disclosure model still works (info open, action gated by attendance). | **done** — satisfied by construction: a Meet-service booking adds the owner to the meet roster via `setMeetRsvp`; paid and free attendees are indistinguishable `MeetAttendee` entries. No People-tab change needed. |
| D4 | **Meet detail page "Linked services" section** — when a meet has linked services, a small section in the meet body lists them (with the same chrome as service cards on the carer's Services tab). Provides a discoverable on-ramp for users who land on the meet first. | **done** — covered by the D2 callout (optional-link meets) + the existing `serviceCTA` "About this service" card (required/legacy meets). Both surface the linked service in the meet body with a route into booking. |
...
| G2 | **Tereza (informal carer + community connector).** Add her optional "Group walk" Care service linked to her Sunday park walk meet (free to join; 200 Kč if she walks your dog specifically). Demos the mixed-roster configuration end-to-end. Owner viewing the meet sees "Join free" primary + service callout. | **done** — A5 added `tereza-group-walk` (Meet-type) linked to `meet-15` (`required: false`). Mixed-roster verified: walkthrough A6, C10, D2. |
