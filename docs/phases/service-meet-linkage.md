...
| H4 | **Link-authoring UI.** A surface for a carer/host to *create* a meet↔Care-service link. Open design question — the user's model is "the meet admin links services," which needs a meet-edit surface (`MeetComposer` is create-only — none exists); a carer-side picker on the Care service edit card is the alternative. The demo seeds the link (H3), so this is the remaining piece. **Multi-carer** linking (a host advertising *other* carers' services) folds in here. | **open** — surface decision pending; demo unblocked by the seeded link. |
