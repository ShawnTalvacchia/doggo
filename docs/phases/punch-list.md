...
| P29 | Soft Familiar indicator across the app on people you've marked. Surfaces: group members, meet attendees, profile cards, anywhere a person row renders. Quiet visual treatment — small icon (e.g., a check or familiar-glyph) tucked near the avatar OR a tertiary-color "Familiar" label next to the name. Privacy-safe (it's your own data; no deniability concern since the mark is from your side). User benefit: scanning a list and seeing "I've marked this person before" without having to remember. Pairs naturally with **P27** (avatar pattern cascade) — both are app-wide person-row enhancements that should ship together. **Out of scope for the post-meet review surface itself** — that has explicit per-section grouping that already conveys mark state. | Design / Trust | App-wide person rows | `components/people/PersonRow.tsx`, `lib/personActions.ts` (matrix outputs `familiar(on)` state which renderers can use), every consumer of PersonRow | 2026-04-27 |
