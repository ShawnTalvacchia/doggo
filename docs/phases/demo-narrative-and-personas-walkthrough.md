...
- [ ] **W1.1. Doc exists at the chosen home.** [docs/strategy/Demo Narrative.md](../strategy/Demo Narrative.md) — frontmatter has `category: strategy`, `status: draft`, `last-reviewed: 2026-05-14`. Lives in `strategy/` per the phase decision (positioning artifact alongside Product Vision, not folded into demo-mode.md).
- [ ] **W1.2. Anchor names a real seeded meet.** Doc names **Klára's Saturday training meet at Stromovka (`meet-care-1`)** as the anchor. Service ↔ Meet linked + paid required. No hypothetical "we'll seed this later" anchor.
- [ ] **W1.3. Five POV beats listed with distinct feature focus.** Beats: (1) Daniel discovery+book, (2) Klára active session with Hana, (3) Magda connect+invite+peer-care, (4) — *Daniel→Klára inquiry tail explicitly cut, see "What's NOT walked"* —, (5) Lena coda. *(Two original Beat-4/5 in older drafts; the doc collapses to four total beats.)* Each beat states: persona, time, surfaces walked, demo focus, tester actions, tester prompt.
...
- [ ] **W1.6. Anchor-day staging table reads as W3 input.** Per-row "New seed?" / "MODIFY" annotations match what actually shipped to mock data (Magda persona NEW, Veronika NEW, group-holesovice-block NEW, meet-care-1 attendees MODIFY, kh-6 MODIFY, connection rosters NEW).
...
- [ ] **W4.4. Mode toggle entry + active chrome + end-state spelled out.** Entry via `/demo` with new "Start guided walkthrough" entry; active-walkthrough chrome is a slim header bar with "Step N/5 · Pause · Exit"; end-of-walkthrough fires a closing interstitial with "Pick another persona" + "Stay as {current}".
- [ ] **W4.5. Pause/Resume mechanic spelled out.** Pausing keeps current persona, switches to Open View, surfaces a slim "Resume walkthrough" pill. Resume re-renders from the paused interstitial. State persists in `sessionStorage`; tab-close ends cleanly.
- [ ] **W4.6. Persona-switch transition has an answer.** Spec picks "interstitial dismissal covers the swap" — fade dismissal (200ms) while new persona's surface mounts. Interstitial entry has subtle avatar scale-in + sequenced text fade. Engineering reality (no real load) acknowledged.
- [ ] **W4.7. Open build-time questions logged.** Spec ends with explicit "Open build-time questions" subsection: beat surface routing registry, atomic-vs-stepped beats, state persistence between beats (a vs b), mid-beat exit handling, mobile-vs-desktop layout. Build phase has clean handoff.
- [ ] **W4.8. Build-phase scope is queue-able from this spec.** A reader can write 3+ build-phase tasks directly from this spec — e.g. "Build interstitial component," "Wire mode toggle in /demo + persistent header bar," "Implement persona-switch transition." No further design decisions needed before tasks become tractable.
