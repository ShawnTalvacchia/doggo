...
- [ ] **O9. The shelter-facing demo drives the storyline LIVE from fresh state** rather than pre-staging Tomáš mid-mentorship (board H1 said "pre-staged"). The state-toggles make live driving fast, and a pre-staged state would collide with demonstrating the booking beat. (See the shelter demo doc's Beat structure.)
