...
- [ ] **A5. `/` care section.** Existing care-split unchanged in copy + photo. Section closes with a **"Browse the prototype" primary pill button** (brand-fill, large) → `/demo`. The CTA is visually elevated and has a 32px gap above it (bigger than the 20px gap between care bullets) so it reads as its own beat, not a continuation of the bullet list. Matches the section-level CTA pattern of Hero / Archetypes / Bottom CTA.
