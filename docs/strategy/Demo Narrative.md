...
**Beat 1 — Eliška finds a way in.** *Surfaces:* `/discover/help-a-dog` (the "walk a shelter dog — no adoption obligation" doorway) → `/shelters/utulek-liben` (she already follows Útulek). *Focus:* explore-before-you-commit; a warm on-ramp beats decoding a cold shelter process. The Saturday group walk is discovered community-side (Klára's training group / meets discovery), not from the shelter page (shelters don't list meets on their page — PO call 2026-06-12). *Hero action:* open the Saturday group walk.

**Beat 2 — The group walk (discovery).** *Surfaces:* `/meets/meet-utulek-group-walk`. *Focus:* a **dog-first** community walk — neighbours bring their own dogs; vouched walkers each pick up a shelter dog on the way (the "Shelter dogs joining" panel). The walk is for people with a dog to handle, so a newcomer doesn't tag along dogless — Eliška **discovers** the walk, sees Nora out with Marie, and the **"Walk with a mentor"** CTA is her way in. The green→violet funnel: a community Meet that produces shelter care. *Hero action:* taps **Walk with a mentor** → Beat 3. *(Reframed 2026-06-13: dropped the "comes along socially / no dog yet" attendance beat — it contradicted the dog-first framing; the mentored first walk, Beat 3, is where she actually participates with a dog.)*
