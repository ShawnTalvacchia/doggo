...
> **Meet variant — the mixed group shelter walk** (Adoption-Curious Journey / FC18, 2026-06-12; sign-up linkage shipped 2026-06-24). A community Meet (green) linked to a shelter via `Meet.shelterWalk?: { shelterId }` so it can source shelter dogs. It's **dog-first and mixed**: most attendees bring their own dog; a walker signs up *on* the trainer-led group walk to walk a shelter dog. Signing up creates a real shelter-walk **`Booking` linked to the meet occurrence via `Booking.dropoffMeetId`** — the same config-#2 linkage documented below, so it rides the existing Sessions / Visit-Report rails (and is captured for the advocacy loop). The single source of the booking shape is `lib/groupWalkBooking.ts` (`buildGroupWalkBookingInput`), shared by the meet-page sign-up and the `MentorSessionBookingSheet` so the two entry points can't drift.
>
> **Two tiers, set by vouch status.** A **vouched** walker's group walk is a FREE volunteer walk (`price` total 0). An **un-vouched** newcomer's group walk *is* their **paid mentored first walk** — it carries the host trainer's per-session fee (same price as a 1-on-1 `mentor_session`; the group format just lets several mentees pay the trainer at once) and **counts toward the shelter's vouch** alongside 1-on-1 mentor sessions (the walker's `committedSessions` count includes meet-linked mentored group walks). So the mentored first walk — group or 1-on-1 — is a paid mentor session advancing the same vouch tracker; only the vouched/plain volunteer walk is free. The Meet stays a community object (green, discovery via the host's care group); `shelterWalk` only sources the dogs (the green→violet funnel boundary). Full surface detail in [[features/shelters]] → "Group shelter walk (FC18)."
