...
**On the People tab disclosure model.** The meet attendee list separates **information** (open) from **action** (gated by *meet-level engagement*). Anyone who can see a meet can see who's attending, grouped by relationship state, with locked attendees shown as a chip list at the bottom. Action affordances (Familiar / Connect / Message pills) render for viewers with a seat in the meet — creators, anyone with any RSVP (past or future, going or interested), and anyone following the series. **Resolved 2026-05-11 (P32):** widened from past-attendance-only so a creator/RSVP'd/following viewer can act on attendees they share the meet with — analogous to the Group Members tab where co-membership IS the action context. This is a People-tab rule, not a content-visibility rule per se — but it shares the underlying principle that opting into a meet is the trust context that unlocks deepening. The Group Members tab applies the same single-principle gating; the locked-profile shared-context gate (`viewerSharedMeetWith`) keeps narrower past-shared-attendance semantics — see Trust & Connection Model for the divergence rationale. Spec: `Trust & Connection Model.md` → "Meet participant visibility rules" + `features/meets.md` → People tab.
