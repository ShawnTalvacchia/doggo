...
| No | — | `participants_only` *(planned)* | — | **No** | Visible only to creator + roster — neither group members nor the public see it. Use case: contracted/package-generated 1-on-1s. See note below. |
| No | No (standalone meet) | — | — | **No** | Only attendees see standalone meet content |

**On the `participants_only` level (planned, not yet implemented).** A third meet visibility level beyond `public | group_only`. Surfaces meets that are technically inside a group but should NOT be discoverable by other group members — specifically: instances generated by a package booking or contracted 1-on-1 (e.g., one of the eight sessions in a client's training package). Today these are stored as `group_only`, which leaks them to other group members on the public Meets tab and creates a "wait, can I book Thursday at 11?" trap. Proper visibility: only creator + roster see the meet at all. Lives on the participants' schedules, never on group/discover surfaces. Tied to the Services-as-Catalog model (see [[Groups & Care Model]] → Services as Catalog → Meet-type service visibility). Implementation: extend `Meet.visibility` enum, update `getGroupMeets` + Meets-tab filter, apply automatically when a meet is generated by a package booking. Tracked in Open Questions §4 + punch-list.
...
**On the People tab disclosure model.** The meet attendee list separates information (open to all who can see the meet) from action (gated by meet-level engagement). This is a People-tab rule rather than a content-visibility rule per se — see [[Trust & Connection Model]] → "Meet participant visibility rules" for the canonical spec.
...
**Profile gallery is filterable:** Filter UI on the Posts tab uses a **+Filter pattern** (Photos & Galleries 2026-06-04). A dashed-border `+ Filter ▾` button opens a menu of available tag types (Dog / Person / Place / Community / Meet); picking a type opens its values picker; selected filters render as active pills on a second row. Multi-select within a type (OR); multiple types AND together. Each type's values dropdown is scoped to viewer-visible posts only — the dropdown lists, the visible-post count, and both view modes (List + Grid) all read from `isPostVisibleTo`-gated posts. Viewers never see a filter label or value for a private group they're not in.

**Implementation note (Photos & Galleries, 2026-06-04).** Auto-album surface on `/dogs/[id]` consumes the gated set via `getPostsByDog(dogId, viewerId)` in `lib/dogPosts.ts`. Owner sees ALL tagged posts on their own dog (owner is the dog's authority); other viewers see only posts passing the two-gate. Both `/dogs/[id]` and `/profile/[id]` Posts tab mount the same `PostsCollectionView` — single source of truth, scoped by dog-tag vs by author. Tags do not expand audience on either surface. The per-post kebab's "Untag {Dog}" action is a per-viewer suppression filter (it doesn't mutate `Post.tags[]` until an editable-post store lands — FC13); the dog's auto-album reads the untag list and excludes hidden posts.
