...
| **Config #2 link authoring + multi-carer** *(follow-on from Service ↔ Meet Linkage — sized 2026-05-17)* | Config #2's owner-facing side (a free meet + a linked drop-off Care service; book ≠ attend) shipped *in* the Service ↔ Meet Linkage phase as Workstream H. This follow-on is the remaining slice: the **link-authoring UI** — a surface for a carer/host to *create* a meet↔Care-service link (the phase seeds it in mock data). Open design question — the user's model is meet-admin-authored, which needs a meet-edit surface that doesn't exist (`MeetComposer` is create-only); a carer-side picker on the Care edit card is the alternative. **Multi-carer** linking (a meet host advertising *other* carers' services) folds in here. | `Open Questions §13` (config #2 correction note), `phases/service-meet-linkage.md` (Workstream H) |
