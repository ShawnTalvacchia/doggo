...
**Service ↔ Meet Linkage** *(opens 2026-05-13)*. The carer authors Care / Meet / Appointment services in one place; the authoring UI, the required-link config (booking = RSVP), and the Meet-service booking flow shipped — currently mid-walkthrough. **Config #2 (optional-link / mixed-roster) was remodelled 2026-05-17**: a drop-off "Group walk" is a Care service, not a Meet-type service, so the optional-link case was reverted and rescoped — its corrected build is the follow-on *Meet-Linked Care Services* phase. Phase board: `phases/service-meet-linkage.md` (see the remodel note).
...
| Phase | Goal | Key refs |
|-------|------|----------|
| **Design System Cleanup** *(recommended next — sized 2026-05-14 from Profiles Deep Pass close)* | Consolidation pile is real: Section shell component (FC4), IdentityChip (FC5), the recurring `flex-1 + white-space:nowrap` CTA pattern, P67 component-consolidation audit, and the bundle-wrapper pattern applied 3+ times during PDP. Doing this between Service ↔ Meet Linkage and Cross-Cutting Flow Testing makes everything that follows cheaper. Not yet drafted — would absorb the FC items, P67, and a sweep of accreted patterns. | `Future Considerations.md` FC4/FC5, `phases/punch-list.md` P67, `docs/implementation/design-system.md` |
| **Meet-Linked Care Services** *(follow-on from Service ↔ Meet Linkage — sized 2026-05-17)* | Builds the **corrected config #2**: a free community meet can link one or more drop-off **Care** services, and an owner books a carer to walk their dog *without* joining the meet roster (**book ≠ attend** — two separate paths: join free as a walker, or book as an absent owner). Scope: Care services made meet-linkable in the data model (today only Meet-type services carry `linkedMeetIds`); a drop-off-from-meet booking flow that produces a Care `Booking` without faking RSVP; the meet-detail callout for Care-linked free walks; multi-service / multi-carer links (a meet host links other carers' services too). Emerged from the Service ↔ Meet Linkage walkthrough when "Group walk" was found miscategorised as Meet-type. | `Open Questions §13` (config #2 correction note), `Groups & Care Model.md`, `phases/service-meet-linkage.md` (remodel note) |
