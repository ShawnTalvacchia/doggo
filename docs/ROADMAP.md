...
**Up next:** the buildable-now phases below — **Company / Organization accounts**, **Grooming** (finish + size-pricing), **Shelter partnership value & give-back** — buildable in any order (the phase-close strategic review recommends a sequence; see the kickoff). With the shelter pitch now complete on both sides (walker journey + operator surfaces) and a feasibility kit ready, **shelter interviews can run in parallel**. A design-system code-health backlog runs on the punch list — P82.
...
**Interviews run in parallel, not as a gate.** The PO is arranging trainer and shelter interviews; they feed the validation horizon (below) but are *not* a blocker for building. Plenty of interview-independent enrichment can proceed meanwhile — that's what most phases below are. Build the world richer while the conversations get scheduled.

The phases below are buildable now, in any order. The governing principle stays **desirability before feasibility**: the demo paints an appealing, clear picture first; the hard feasibility questions — insurance, liability, the binding-constraint — come later, in conversation.

| Phase | Goal | Key refs |
|-------|------|----------|
| **Company / Organization accounts** — *buildable now* | A new institutional account type for businesses that aren't individuals — streamlined, helpful chrome with no meets or community-service surfaces, tuned to what a business actually needs. **Shelters are the precedent** (a top-level institutional entity with repurposed page chrome); this generalizes that pattern to commercial orgs. **First instance: grooming salons.** Individuals offering the same service stay plain Carers (a specialization, no chrome change) — the account type is only for true organizations. Carries the org's **existing web presence** (website + socials, as shelters already do via `ShelterProfile.website`/`socialLinks`) so Doggo reads as **additive exposure, not a replacement** for a business's current site — a direct onboarding-friction reducer (a business doesn't have to abandon what it has to join). Strategic payoff: onboarding real businesses is a **cold-start lever** and makes the platform feel fully fleshed-out and inclusive. | `features/shelters.md` (institutional-entity precedent), CLAUDE.md → Identity & Architecture (the account-model principles this extends), `strategy/Cold-Start Playbook.md` |
| **Grooming service — finish the build + size-based pricing** — *buildable now* | Grooming is half-built (Appointment-type service, `groomer` specialization, Mánesova salon mock, identity badges, Discover support) but not fleshed out or demoed. Complete it and add the dimension the model lacks: **size-based intake + pricing**, scoped to groomers — per PO interviews, trainers deliberately *don't* vary by dog size, groomers do, so it's a **groomer-scoped pricing modifier**, not a global one. Serves individual Carer-groomers and salon (company) accounts alike. Demo arc deferred; persona switcher for now. | `features/explore-and-care.md` (services/pricing), `lib/pricing.ts` (modifiers — add a size dimension), `lib/types.ts` (`AppointmentCategory: grooming`) |
| **Shelter partnership value & give-back — contribution beyond walks** — *buildable now* | Sharpen and *demonstrate* why a shelter benefits from partnering, and give users non-walk ways to help: **sponsor-a-dog** and **donate** surfaces. Turns the documented give-back direction into a demoed surface that completes the shelter pitch (walks + advocacy + sponsorship + donation + adoption funnel). Phase 2 leaves give-back interview-gated; this builds the demo-facing version. **Partnership-value point:** Doggo as *additive exposure* — more eyeballs on a shelter's dogs and its existing site/socials, not a site to migrate into — is part of the why-partner argument here. | `strategy/Doggo_Positioning_and_Three_Worlds.md` (give-back model), `features/shelters.md`, `Open Questions §6`, `Future Considerations` FC16 |
...
**The validation horizon.** Everything above builds toward one threshold — not a next phase: putting the demo in front of real users, trainers, and at least one shelter, then making the **build-for-real** call (backend, who builds it, the give-back/legal structure). That's the "get serious" gate between this prototype and a product. More enrichment phases may land before we reach it — it's the *direction*, not the next item on the list. → `strategy/Doggo_Positioning_and_Three_Worlds.md` (§Where this goes next), `Open Questions & Assumptions Log.md`, and *Beyond the Demo* below.
