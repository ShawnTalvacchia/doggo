...
| **Config #2 multi-carer linking** *(follow-on from Service ↔ Meet Linkage — sized 2026-05-17)* | Config #2 shipped end to end *in* the Service ↔ Meet Linkage phase (Workstream H) — owner-facing callout + drop-off booking + carer-side link authoring. This follow-on is the one deferred slice: **multi-carer** linking — a meet host advertising *other* carers' drop-off services on their meet (today a carer links only their own services, from their Care service card). It needs a meet-side authoring surface, which doesn't exist (`MeetComposer` is create-only) — a meet-edit screen would be the home. | `Open Questions §13` (config #2 correction note), `phases/service-meet-linkage.md` (Workstream H4) |
