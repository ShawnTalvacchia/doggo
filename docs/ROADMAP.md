...
**Up next:** **Multi-Path Demo — Three Community Worlds** is open (Phase 1 of a two-phase validation arc; see *What's Next*). A design-system code-health backlog runs on the punch list — P82.
...
The **two-phase validation arc** (scope confirmed 2026-06-22). FC17's documented four-path taxonomy was reconciled against the PO's three-audience framing: the three community Worlds become Phase 1; the shelter's *own* (operator) view — which existed nowhere as a walkthrough, only faked toggles + FC16 — graduates into Phase 2. Governing interview principle: **desirability before feasibility** (the demo paints an appealing, clear picture first; the hard feasibility questions — insurance, liability, the binding-constraint — come after, in conversation).

| Phase | Goal | Key refs |
|-------|------|----------|
| **Multi-Path Demo — Three Community Worlds (FC17)** — *Phase 1, now open* | Three tight, single-thesis guided walkthroughs over one shared world, each **doubling as a one-audience interview artifact** ("the guided demo IS the interview"): **W1 new owner→community** (Daniel; absorbs neighbour-care), **W2 trainer/walker→community+living** (Klára), **W3 shelter→adopted, demand-side** (Tomáš mentee + Eliška advocacy→adoption). Plus the two real redesigns: launcher **journey doors** (not one CTA) + a named-**walkthrough registry**. Paths share a world and may overlap. | `Future Considerations` FC17, `features/demo-mode.md`, `strategy/Demo Narrative.md`, `strategy/Doggo_Positioning_and_Three_Worlds.md`, `strategy/mentor-network-shelter-demo.md` |
| **The Shelter's Side — operator experience + feasibility kit** — *Phase 2* | The shelter's *own* POV — "what do I do, how little work/risk is this": the **handover** (check-out → back-safe check-in, logged), the **vouch/application landing**, **walker-pool** view, **adoption-interest landing**. Built as deep as makes the strongest impression; graduates a defined slice of **FC16**, rest stays faked/future. Completes the shelter pitch (both sides). Ships the **feasibility interview kit** (desirability-first; the questions the demo can't ask). | `Future Considerations` FC16, `strategy/Doggo_Positioning_and_Three_Worlds.md` (§3 + Appendix), `strategy/mentor-network-shelter-demo.md`, `Open Questions §5 / §14 / §6` |
