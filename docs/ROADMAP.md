...
Reorganized 2026-06-03 after Dog Profile close. **Photos & Galleries is next** — Dog Profile shipped the per-dog photo landing slot (`Coming soon — photos from posts tagging {Dog} will surface here`) and made `/dogs/[id]` the canonical surface, so Photos & Galleries lands its auto-album work into surfaces that are already in place. Prior shifts retained: (a) **shelter direction graduated** to an active strategic thread (Shelter Foundation shipped 2026-06-02), (b) **Dog Profile shipped 2026-06-03** (archived board) — established the unified `/dogs/[id]` surface across shelter + owned dogs, shipped Vaccines V1 + standing preferences + FC8 tag taxonomy formalization + PetCard → photo-led summary tile pattern (Workstream G), (c) **Carer Portfolio and Shelter Walker Credentialing merge** into one credentialing-moat phase.

| Phase | Goal | Key refs |
|-------|------|----------|
| **Photos & Galleries** *(next — unblocked 2026-06-01 §12 resolved; landing slot shipped by Dog Profile 2026-06-03)* | Per-dog "Photos" auto-album derived from tagged posts; profile-level photo grid; curated Highlights strip; owner moderation (untag via per-post menu / hide-from-album + pin-to-Highlights via long-press / per-dog settings); cross-pet households (per-dog albums on each PetCard). Tag-approval inherits from owner (per-dog override deferred). Dog Profile shipped the landing slot + made `/dogs/[id]` the canonical surface — Photos & Galleries plugs the auto-album machinery into that surface. | `phases/photos-and-galleries.md` (draft), `Content Visibility Model.md`, `Open Questions §12`, `archive/phases/dog-profile.md` (landing slot context) |
