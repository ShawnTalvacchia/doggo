...
| **Multi-Path Demo & Guided Walkthroughs (FC17)** *(future; the Adoption-Curious *content* half shipped 2026-06-13, this is the packaging half split off)* | The guided-UI layer over the demo arcs. The content is built and drivable in Open View (`?as=`): the V2 Daniel→Klára thread, the shelter-mentor arc, and now the **adoption-curious arc** (Eliška — beats documented in `Demo Narrative.md` → Adoption-Curious Arc). FC17's job is to **wrap** these arcs in guided UI — interstitial/step-card registry, per-arc staging, the `/` launcher doorway restructure, and "cut each path tighter" so multiple paths stay streamlined — NOT to design the arcs (each arc's beats are a clean handoff: wrap, don't redesign). Registry id `adoption-curious` ("Try before you adopt") source → built. | `features/demo-mode.md`, `strategy/Demo Narrative.md` (arc beats), `Future Considerations.md` FC17 |
