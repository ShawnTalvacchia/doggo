...
6. **Pet-as-protagonist (Sessions & Service Execution, 2026-05-08; full-hero retired from the Sessions tab, Multi-Path Demo & Guided Walkthroughs, 2026-06-22).** For surfaces that frame care of a specific dog, the dog leads. The original treatment was a full-width hero photo over avatar-as-thumbnail — meant to encourage emotional bonding, instant recall, and a behavioral nudge for owners to upload quality photos. **Revised:** on the booking detail Sessions tab + active sub-page, the full hero stretched landscape photos into a thin band and caused a header swap on Start session, so `SessionsPetHeader` now renders its **`compact`** variant everywhere (dog still leads, avatar-left, stable across both surfaces). The dog-as-centerpiece principle still holds; the *full-bleed hero* shape did not. See SessionsPetHeader variants below.

   **`SessionsPetHeader` variants** (`components/bookings/SessionsPetHeader.tsx`):
   - **`compact`** — avatar-left (140px rounded-square dog photo per Avatar Rule B) + name/meta to the right, `gap-xl`, dog-profile-header shape. **Used everywhere now** — both the Sessions tab (`app/bookings/[bookingId]/page.tsx`) and the active/live sub-page (`app/bookings/[bookingId]/active/page.tsx`) pass `variant="compact"`, so Start session keeps the header stable and only the body changes; on the live page it lifts the working actions (Log GPS / Add a note / Finish) above the fold.
   - **`full`** — full-width rounded photo (`max-height` clamp) + 24px name heading, the original pet-as-protagonist hero. **Retained in code but currently unused;** kept as the alternative shape (remove if compact-everywhere sticks).
...
| `CheckboxRow` | Labeled checkbox | `checked`, `label`, `placement`, `density` (`default` / `compact` — compact = 24×24 indicator + 8px gap + 14px check for dense forms; replaced the `.pet-edit-checkbox-row` descendant override, P78) |
| `SortMenu` | Custom-styled sort dropdown (label + caret trigger + `.dropdown-menu` listbox, outside-click + Esc) | `value`, `options` (`{value,label}[]`), `onChange`. Extracted from two byte-identical inline copies (shelter Dogs tab + Help a Dog); CSS `.sort-menu-trigger`/`.sort-menu-wrap` (renamed from `.shelter-sort-*`). FC15 / Design-System Audit WS-D. |
