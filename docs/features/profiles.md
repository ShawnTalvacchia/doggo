...
- **Horizontal hero inside the About tab body** (Profiles Deep Pass 2026-05-11): avatar (200×200) on the left, name (persona dropdown trigger) + Carer identity badge + location + dogs + member-since + Share Profile button stacked on the right. Stacks vertically on narrow viewports. Matches `/profile/[userId]` hero shape so own-vs-other look the same. Hero lives inside the About tab — Posts and Services no longer show it above their content. (Previous centered hero above tabs was removed because the layout diverged from other-user profiles and the "Edit Profile" affordance felt orphaned from its Save/Cancel chrome.)
...
- **Connections list — compact + Show all** (Profiles Deep Pass 2026-05-11). Each state group (Connected / Familiar / Pending) caps at 5 rows in the compact view; if any group exceeds the cap, a "Show all (N) →" button appears below the groups and opens a `ModalSheet` titled "Connections · N" with the uncapped grouped list. Same `ConnectionRow` JSX shared between compact + modal. No dedicated `/connections` route — scope kept tight for the prototype.
- **Posts tab:** `ShareMomentBar` prompt strip at the top of the panel (Profiles Deep Pass B5, 2026-05-11). Full-width `--surface-top` strip with `--border-strong` top + bottom borders, hosting a sunken `--surface-inset` pill input ("[Camera] Share a moment..."). AppNav Camera+ is suppressed here via `suppressCreate` — the strip is the affordance. Replaces the prior floating "+ New post" button which read as awkward and duplicated the AppNav icon.
- **Services tab:** editable services, availability, audience toggle. **Comprehensive catalogue** — both Care-type offerings (Walking, Sitting, Boarding) and Meet-type offerings (Training sessions, Workshops, paid Group meets) live here in one list. See [[Groups & Care Model]] → Services as Catalog for the split.
...
- **Own-self redirect** (Profiles Deep Pass 2026-05-11). Visiting `/profile/<ownId>` while logged in as that user (e.g. typing your own URL) redirects to `/profile`. Without this the page applies the viewer-vs-subject lock gate to your own self — locked personas see "this profile is private" about their own profile. Tab param is preserved when the destination tab exists on the own-profile route (about / posts / services); dropped otherwise.
