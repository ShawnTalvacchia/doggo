...
- **Services tab:** editable services, availability, audience toggle. **Comprehensive catalogue** — Care-type offerings (Walking, Sitting, Boarding), Meet-type offerings (Training sessions, Workshops, paid Group meets), and Appointment offerings (grooming / training visits) all live here in one list. See [[Groups & Care Model]] → Services as Catalog for the split.
  ...
  - **Shared service-card renderer (Design-System Audit WS-A, 2026-06-21).** The own-profile Services *preview* and the viewer-facing `/profile/[userId]` Services tab both render through `components/profile/ServiceCardViews.tsx` (per-kind `Care/Meet/Appointment/Mentor` views + an optional `action` slot for the Book/Ask CTA). Single source of truth — fixes a drift where the own-profile Care card had lost the "From" prefix + the pickup/drop-off and full/half-day priced-axis breakdown, showing a wrong flat price the public never saw. Kind-order (Appointment → Mentor → Care → Meet) and chip chrome now match the viewer surface, since the own-profile view is defined as the "what owners see" preview.
