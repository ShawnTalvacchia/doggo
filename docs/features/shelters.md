...
- Stages persist via the demo override `useAdoptionStore` (dogId → `interested | pending | adopted`); shelter-side advances are honest state-toggles (real operator surface = FC16). `pending` surfaces the "Adoption pending" hero pill (solid violet) + a matching glass-violet card chip on Discover / the shelter Dogs tab — both color-coded to the shelter / adoption-journey violet accent (the mentor link, doorway card, credential pills); `adopted` → green, the celebration below. The **primary** finalize is the **network adopter** (tied to the walk-recap commenter — the advocacy loop closed); self-adopt is secondary, per the research (community exposure is the proven engine).
