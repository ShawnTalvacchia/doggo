...
**Demo-scope pivot (PO, 2026-06-21).** "Move quickly — this is a demo, not the most important step; trust your call + verification on most of it." Acted on: drove the rest myself, self-verifying visible changes on the dev server, and **deferred the invisible/low-demo-value code-health tail** rather than grinding it. Deferred-for-demo (all are sound but not worth the cycles/risk for a demo): **C3** dropdown-hoist (cascade risk, invisible), **C4 + WS-H** type-scale reconciliation + the 294-site font-size migration (app-wide *visual* risk, zero demo payoff — the one item I won't touch without a code-health reason), **C6** styleguide drift (internal page, invisible), **D1/FC4** Section shell (invisible refactor), **F3/F4/F5** selection-control + connection-chip cleanups (invisible, per-site), **E2/E3** card-shell primitive + section-header routing (invisible, 37 sites), **WS-G** inline-style sweep (tedious per-site className merges, invisible). All remain accurately mapped on this board for a future code-health pass.
...
| F2 | **Done + self-verified on the dev server.** Converged ScheduleCard status (Hosting/Joining/Providing/Care/Cancelled) from corner chip-pills to inline icon + colored text (Principle 7) — green for meet roles, blue for care, error-red for cancelled. Removed the dead `.sched-card-role*`/`.sched-card-tag*` CSS + the unused `isHosting`. | [[design-system]] (Principle 7) | done |
| F3 | **Deferred (demo-scope).** Single-select primitive for the One-time/Repeat picker — invisible refactor, no demo value. | [[design-system]] | deferred |
| F4 | **Deferred (demo-scope).** `.pill-group` → `PillToggle` — `PillToggle` is label-only; the ~7 sites carry icons/custom content + vary (single vs multi). Per-site, invisible, no demo value. | [[design-system]] | deferred |
| F5 | **Deferred (demo-scope) — call made.** The Discover-card-inline vs PersonRow-class "drift" is intentional (Connected earns a chip on roomy cards, not on dense rows where the Message CTA carries it). Only cleanup would be inline-style→class on the Discover chip — invisible, marginal. Left as-is. | [[punch-list]] P51, [[Trust & Connection Model]] | deferred |
