...
| D4 | Carer audience signal + provider stats prominence — now that the Care-offering picker (C6, 2026-05-11) replaced the "Open to helping" pill, the Carer Identity badge on the hero is the sole carrier of role + audience. Provider stats (rating, reviews, completed engagements once Carer Portfolio phase opens) remain a separate prominence question. Deferred pending PO outcomes. | todo |
...
| D7 | **Carer audience toggle UI (2026-05-11).** Shipped, then superseded by C6's care-offering picker (2026-05-11). Original scope: raw `<button>` for the public-visibility setting was replaced with a real `Toggle` matching the Open-to-helping pattern. C6 then folded both Toggles ("Open to helping" + "Open to anyone") into a single radio-card picker with three explicit options (Not offering / Connected circle only / Open to anyone) — see the C6 entry in the walkthrough's Decisions section. The audience setting is still `carerProfile.publicProfile`; the UI surface is the new picker. `onToggleVisibility` prop signature stays `(v: boolean) => void`. All ladder/tier/escalation language dropped per the Discover Refinement collapse. | done |
