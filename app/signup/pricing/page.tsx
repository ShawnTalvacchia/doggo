...
function prevStep(roles: string[]): string {
  ...
  return "/signup/care-preferences";
}

// Forward-destination: pet step (owners) or success
function nextStep(roles: string[]): string {
...
function PriceInput({
  ...
  const { Icon, label, unit, hint, marketRange } = config;

  return (
    <div className="pricing-service-row">
      <div className="pricing-service-info">
        <div className="pricing-service-title">
          <Icon size={18} weight="duotone" className="pricing-service-icon" />
          <strong>{label}</strong>
        </div>
        <p className="pricing-service-hint">{hint}</p>
        <p className="pricing-market-range">{marketRange}</p>
      </div>
      <div className="pricing-input-group">
        <div className="pricing-input-wrap">
          ...
          />
          <span className="pricing-input-unit">Kč</span>
        </div>
        <span className="pricing-input-label">{unit}</span>
      </div>
    </div>
  );
}
...
export default function SignupPricingPage() {
  ...
  const stepInfo = getStepInfo("pricing", draft.roles);
  ...
  // Must set a price for each active service to continue
  const canContinue = activeServices.every(
    (cfg) => draft.prices[cfg.type] !== null && (draft.prices[cfg.type] ?? 0) > 0,
  );

  const setPrice = (type: ServiceType, value: number | null) => {
    updateDraft({ prices: { ...draft.prices, [type]: value } });
  };
    ...
    <main className="page-shell">
      <div className="form-shell">
        <FormHeader
          ...
          subtitle="Set a starting price for each service. You can update these anytime from your profile."
          step={stepInfo?.step}
              ...
              <div className="pricing-services-list">
                {activeServices.map((cfg) => (
                  <PriceInput
                    key={cfg.type}
                    config={cfg}
                    value={draft.prices[cfg.type]}
                    onChange={(v) => setPrice(cfg.type, v)}
                  />
                ))}
              </div>
            </div>
        ...
        <FormFooter
          onBack={() => router.push(prevStep(draft.roles))}
